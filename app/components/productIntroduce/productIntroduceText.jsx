//线上服务文案
const lineTops = [
            {
              title: '薪酬管理',
              img: "../../images/productIntroduce/u1154.svg",
            },
            {
              title: '社保福利',
              img: '../images/productIntroduce/u1159.svg',
            },
            {
              title: '奖金智能核算',
              img: '../images/productIntroduce/u1164.svg',
            },
            {
              title: '工时考勤',
              img: '../images/productIntroduce/u1170.png',
            },
            {
              title: '统计报表',
              img: '../images/productIntroduce/u1174.svg',
            },
            {
              title: '公司与员工管理',
              img: '../images/productIntroduce/u1224.png',
            },
            {
              title: '权限管理',
              img: '../images/productIntroduce/u1185.svg',
            }
          ];
          //线下服务文案
const lineBottoms = [
            {
              title: '社保代缴',
              img: '../images/productIntroduce/u1178.png',
            },
            {
              title: '工资代发',
              img: '../images/productIntroduce/u1181.svg',
            },
            {
              title: '资金安全',
              img: '../images/productIntroduce/u1186.png',
            },
            {
              title: '福利商城',
              img: '../images/productIntroduce/u1262.png',
            },
            {
              title: '增值服务',
              img: '../images/productIntroduce/u1192.png',
            }
          ];
const texts = [
            {
              title: '社保代缴',
              content: "一站式服务，让您在全国所有城市的社保公积金委托瞬间完成"
            },
            {
              title: '工资代发',
              content: "全国任何城市工资代发任何银行卡工资代发个税代为申报与多家银行深度合作，员工VIP卡，无限尊享"
            },
            {
              title: '资金安全',
              content: "银行托管，专款专用与平安保险，中国人保合作，风险为零"
            },
            {
              title: '福利商城',
              content: "各类精品为您打造企业级文化团队建设的开端"
            },
            {
              title: '增值服务',
              content: "组织架构薪酬架构设计咨询社保公积金成本异地缴交设计与成本节省HR实务、政策培训与咨询"
            }
          ];
export {lineTops,lineBottoms,texts};